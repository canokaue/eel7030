...
INICIO2:	MOV		IE,#10010000B			;habilita a ocorrência de interrupção serial
			MOV		SCON,#01010000B			;pela frequência desejada, é preciso utilizar o serial no modo 1, REN=1
			MOV		TMOD,#00100000B			;pela frequência desejada, é preciso utilizar timer1 no modo 2
			MOV		TH1,#0FDH				;valor tabelado para obtermos 19.2kibi 
			MOV		TL1,#0FDH				;valor tabelado para obtermos 19.2kibi
			MOV		PCON,#10000000B			;para 19.2 kibi, smod=1
			
			CLR		STATE1					;zera a variável de teste de envio de dados
			CLR		STATE2					;zera a variável de teste para recebimento de dados
			MOV		P2,#00H					;MSB do endereço de memória externa a ser usado
			MOV		R0,#00H					;r0 vai apontar para o endereço solicitado pelo exercício
...
VOLTA:		JNB		STATE1,VOLTA1			;aqui nesse trecho o programa fica alternando entre as linhas 69 e 71 para quebrar o ciclo			
			JMP		ENVIADADOS				;na linha 69 (caso seja momento de enviar dados) ou na linha 71 (caso seja momento de receber
VOLTA1:		JNB		STATE2,VOLTA			;os dados)
			...
			MOVX	@R0,A					;envia o dado para o endereço da memória interna da vez
			INC		R0						;incrementa r0 para enviar o dado para o próximo endereço da próxima vez
			CJNE	R0,#09H,VOLTA			;verifica se já não chegou no endereço #35h
			MOV		R0,#00H					;se já tiver terminado, recarrega o r0 para ficar cíclico
